...
class ResNet_locate(nn.Module):
    ...
    def load_pretrained_model(self):
        self.resnet.load_state_dict('/home/caoyiwen/pretrained_weights/resnet50-timm.pth', strict=False)

    def forward(self, x):
        x_size = x.size()[2:]
        xs = self.resnet(x)

        xs_1 = self.ppms_pre(xs[-1])  # 进金字塔模型前的一个卷积
        xls = [xs_1]
        ...
        return xs, infos  # resnet的输出和金字塔模型输出

def resnet50_locate():
    model = ResNet_locate(Bottleneck, [3, 4, 6, 3])
    model.load_state_dict(torch.load('/home/caoyiwen/pretrained_weights/resnet50-timm.pth'), strict=False)
    return model
