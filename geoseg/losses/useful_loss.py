...
class ABCLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 3:
            ...
            loss = self.main_loss(logit_main, labels) + 0.2*self.aux_loss1(logit_aux1, labels) + 0.2*self.aux_loss2(logit_aux2, labels)
        else:
            loss = self.main_loss(logits, labels)
...
class NewULoss(nn.Module):
    ...
    def __init__(self, ignore_index=255):
        ...
        self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                                   DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
        self.aux_loss1 = FocalLoss(ignore_index=ignore_index)
        self.aux_loss2 = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.aux_loss3 = JointLoss(FocalLoss(ignore_index=ignore_index), SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index))

    def forward(self, logits, labels):
        if self.training and len(logits) == 4:
            logit_main, logit_aux1, logit_aux2, logit_aux3 = logits
            loss = self.main_loss(logit_main, labels) + 0.1*self.aux_loss1(logit_aux1, labels) \
                   + 0.3*self.aux_loss2(logit_aux2, labels) + 0.6*self.aux_loss3(logit_aux3, labels)
        else:
            loss = self.main_loss(logits, labels)
...
class LANLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux1, logit_aux2 = logits
            loss = self.main_loss(logit_main, labels) + 0.3*self.aux_loss1(logit_aux1, labels) + 0.3*self.aux_loss2(logit_aux2, labels)
        else:
            loss = self.main_loss(logits, labels)
