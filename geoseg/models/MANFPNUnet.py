...
# unet0加C4经FPN后进空间注意力模块 80epoch -v2 862 130 863
...
class PoolUNet(nn.Module):
    def __init__(self, n_classes, backbone='resnet18',bilinear=False):
        ...
        self.attention = nn.Sequential(PAM_Module(128),
    ...
    def forward(self, x):
        ...
        fp = self.ppm(c4)
        fa = self.attention(fp)
        x = self.up1(c4, c3)
        p1 = torch.cat([x, Upsample(fa, csize[2])], dim=1)
        x = self.up2(self.uni1(p1), c2)
        p2 = torch.cat([x, Upsample(fa, csize[1])], dim=1)
        x = self.up3(self.uni2(p2), c1)
        p3 = torch.cat([x, Upsample(fa, csize[0])], dim=1)
