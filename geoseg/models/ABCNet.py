...
class ABCNet(nn.Module):
    def __init__(self, band=3, n_classes=8, pretrained=True):
        ...
        if self.training:
            self.conv_out16 = Output(256, 128, n_classes, up_factor=8)
            # self.conv_out16 = Output(256, 128, n_classes, up_factor=8)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            feat_out16 = Upsample(feat_out16, imsize)
            feat_out32 = self.conv_out32(feat_cp32)
            feat_out32 = Upsample(feat_out32, imsize)
            return feat_out, feat_out16, feat_out32
            # return feat_out # 测试关掉
        # feat_out = feat_out.argmax(dim=1) # 测试关掉
        return feat_out
