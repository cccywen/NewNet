...
class Fusion_Module(nn.Module):
    ...
    def forward(self, x, fb, ff):
        x = F.interpolate(x, fb.size()[2:], mode='bilinear', align_corners=True)
        x = self.conv1(x)
        x_guide = torch.mul(x, fb)  # 对应位置相乘

        x_guide = F.interpolate(x_guide, ff.size()[2:], mode='bilinear', align_corners=True)
